...
class SeedCompleteMcqData extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'seed:complete-mcq {--user-id= : ID of the user to create questions for} {--partner-id= : ID of the partner to create questions for}';
     ...
     * @var string
     */
    protected $description = 'Create subjects, topics, and seed 100 MCQ questions for a specific user and partner';
    ...
    public function handle()
    {
        ...
        $this->info("Creating complete MCQ data for:");
        ...
        // Create subjects for the partner
        ...
        foreach ($subjectData as $subjectInfo) {
            $subject = Subject::create($subjectInfo);
            $subjects[] = $subject;
        ...
        $topicData = [
            'Mathematics' => [
                ['name' => 'Algebra', 'code' => 'MATH_ALG', 'chapter_number' => 1],
                ['name' => 'Geometry', 'code' => 'MATH_GEO', 'chapter_number' => 2],
                ['name' => 'Calculus', 'code' => 'MATH_CAL', 'chapter_number' => 3],
                ['name' => 'Statistics', 'code' => 'MATH_STAT', 'chapter_number' => 4],
            ],
            'Physics' => [
                ['name' => 'Mechanics', 'code' => 'PHY_MECH', 'chapter_number' => 1],
                ['name' => 'Thermodynamics', 'code' => 'PHY_THERM', 'chapter_number' => 2],
                ['name' => 'Electromagnetism', 'code' => 'PHY_EM', 'chapter_number' => 3],
                ['name' => 'Optics', 'code' => 'PHY_OPT', 'chapter_number' => 4],
            ],
            'Chemistry' => [
                ['name' => 'Inorganic Chemistry', 'code' => 'CHEM_INORG', 'chapter_number' => 1],
                ['name' => 'Organic Chemistry', 'code' => 'CHEM_ORG', 'chapter_number' => 2],
                ['name' => 'Physical Chemistry', 'code' => 'CHEM_PHYSCHEM', 'chapter_number' => 3],
                ['name' => 'Analytical Chemistry', 'code' => 'CHEM_ANAL', 'chapter_number' => 4],
            ],
            'Biology' => [
                ['name' => 'Cell Biology', 'code' => 'BIO_CELL', 'chapter_number' => 1],
                ['name' => 'Genetics', 'code' => 'BIO_GEN', 'chapter_number' => 2],
                ['name' => 'Ecology', 'code' => 'BIO_ECO', 'chapter_number' => 3],
                ['name' => 'Evolution', 'code' => 'BIO_EVOL', 'chapter_number' => 4],
            ],
            'English' => [
                ['name' => 'Grammar', 'code' => 'ENG_GRAM', 'chapter_number' => 1],
                ['name' => 'Literature', 'code' => 'ENG_LIT', 'chapter_number' => 2],
                ['name' => 'Composition', 'code' => 'ENG_COMP', 'chapter_number' => 3],
                ['name' => 'Vocabulary', 'code' => 'ENG_VOCAB', 'chapter_number' => 4],
            ],
        ];

        foreach ($subjects as $subject) {
            if (isset($topicData[$subject->name])) {
                foreach ($topicData[$subject->name] as $topicInfo) {
                    // Check if topic already exists
                    $existingTopic = Topic::where('code', $topicInfo['code'])->first();
                    if ($existingTopic) {
                        $topics[] = $existingTopic;
                        $this->info("Topic already exists: {$existingTopic->name} for {$subject->name}");
                    } else {
                        $topic = Topic::create([
                            'subject_id' => $subject->id,
                            'name' => $topicInfo['name'],
                            'code' => $topicInfo['code'],
                            'description' => "Chapter {$topicInfo['chapter_number']}: {$topicInfo['name']}",
                        ...
                        $topics[] = $topic;
                        $this->info("Created topic: {$topic->name} for {$subject->name}");
                    }
                }
            }
        }
        ...
        $this->info('Successfully created complete MCQ data!');
        $this->info("Created {$subjects->count()} subjects");
        $this->info("Created {$topics->count()} topics");
        $this->info("Created 100 MCQ questions");
