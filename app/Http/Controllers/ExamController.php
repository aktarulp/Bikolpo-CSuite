...
class ExamController extends Controller
{
    ...
    public function assignStudents(AssignStudentsDTO $request, Exam $exam)
    {
        ...
        try {
            $this->examManagementService->assignStudentsToExam($exam, $request->validatedDTO());
            ...
            return response()->json([
                'error' => 'Failed to assign students: ' . $e->getMessage()
            ], 500);
        }
    }

    public function generateAccessCodes(Request $request, Exam $exam)
    {
        $partnerId = $this->getPartnerId();
        ...
        $request->validate([
            ...
            'batch_id' => 'nullable|exists:batches,id,partner_id,' . $partnerId
        ]);

        try {
            $codes = $this->examManagementService->generateAccessCodes(
                $exam,
                $request->input('count'),
            ...
            return response()->json([
                'error' => 'Failed to generate access codes: ' . $e->getMessage()
            ], 500);
        }
    }

    public function revokeAccessCode(Exam $exam, ExamAccessCode $accessCode)
    {
        $partnerId = $this->getPartnerId();

        if ($exam->partner_id !== $partnerId || $accessCode->exam_id !== $exam->id) {
            abort(403);
        }
        ...
        try {
            ...
            return response()->json([
                'error' => 'Failed to revoke access code: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function printPaper(PaperParametersDTO $request, Exam $exam)
    {
        ...
        try {
            $pdf = $this->pdfGeneratorService->generateExamPaper($exam, $params);
            
            return $pdf->download("exam-paper-{$exam->id}.pdf");
        } catch (\Exception $e) {
            return back()->with('error', 'Failed to generate exam paper: ' . $e->getMessage());
        }
    }
