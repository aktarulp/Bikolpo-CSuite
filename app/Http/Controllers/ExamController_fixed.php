...
// use App\Models\QuestionSet; // Removed as part of migration to exam_questions table
...
class ExamController extends Controller
{
    ...
    public function create()
    {
        ...
        // $questionSets = QuestionSet::where('partner_id', $partnerId)->get(); // Removed as part of migration to exam_questions table
        $topics = Topic::where('partner_id', $partnerId)->get();

        return view('partner.exams.create', compact('courses', 'subjects', 'questionTypes', 'topics'));
    }
    ...
    public function edit(Exam $exam)
    {
        ...
        return view('partner.exams.edit', compact('exam', 'courses', 'subjects', 'questionTypes', 'topics', 'assignedQuestions'));
    }
...
<body>
    ...
    public function storeAssignedQuestions(Request $request, Exam $exam)
    {
        try {
            ...
            $questionNumbers = $request->input('question_numbers', []);

            // Ensure question marks and numbers have the same length as question IDs
            if (count($questionMarks) !== count($questionIds) || count($questionNumbers) !== count($questionIds)) {
                return response()->json(['error' => 'Question marks and numbers must match the number of questions.'], 422);
            }
            ...
            foreach ($questionIds as $index => $questionId) {
                $syncData[$questionId] = [
                    'marks' => $questionMarks[$index] ?? 1,
                    'order' => $questionNumbers[$index] ?? $index + 1,
                ];
            }

            // Sync the questions with the exam
            $exam->questions()->sync($syncData);

            return response()->json(['success' => true, 'message' => 'Questions assigned successfully.']);

        } catch (\Exception $e) {
            \Log::error('Error assigning questions: ' . $e->getMessage(), [
                'exam_id' => $exam->id,
                'partner_id' => $this->getPartnerId(),
                'request_data' => $request->all()
            ]);

            return response()->json(['error' => 'Failed to assign questions: ' . $e->getMessage()], 500);
        }
    }
