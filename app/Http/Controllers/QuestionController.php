...
class QuestionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $subjects = Subject::where('status', 'active')->where('flag', 'active')->where('partner_id', $partnerId)->get();
    ...
    public function create()
    {
        $partnerId = $this->getPartnerId();
        $courses = Course::where('status', 'active')->where('partner_id', $partnerId)->get();
        $subjects = Subject::where('status', 'active')->where('flag', 'active')->where('partner_id', $partnerId)->with('course')->get();
    ...
    public function generateSampleMcqs(Request $request)
    {
        ...
        $subjectsByCourse = \App\Models\Subject::where('status', 'active')->where('flag', 'active')->where('partner_id', $partner->id)->get()->groupBy('course_id');
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $subject = $subjects->isNotEmpty() ? $subjects->random() : \App\Models\Subject::where('status', 'active')->where('flag', 'active')->inRandomOrder()->first();
    ...
    public function seedMcqQuestions(Request $request)
    {
        ...
        try {
            ...
            $subjectsByCourse = Subject::where('status', 'active')->where('flag', 'active')->where('partner_id', $partner->id)->get()->groupBy('course_id');
            ...
            for ($i = 1; $i <= $count; $i++) {
                $course = $courses->random();
                $subjects = $subjectsByCourse->get($course->id) ?? collect();
                $subject = $subjects->isNotEmpty() ? $subjects->random() : Subject::where('status', 'active')->where('flag', 'active')->inRandomOrder()->first();
                $topics = $subject ? ($topicsBySubject->get($subject->id) ?? collect()) : collect();
    ...
    /**
     * Display a common view for any question type with comprehensive analytics
     */
    public function commonView(Question $question)
    {
        ...
        // Get comprehensive analytics for this question
        $analytics = \App\Models\QuestionStat::getQuestionDetailedAnalytics($question->id);
    ...
    /**
     * Show the question download form
     */
    public function downloadForm(Request $request)
    {
        $partnerId = $this->getPartnerId();
        
        // Get courses for the partner
        $courses = Course::where('partner_id', $partnerId)
            ->orderBy('name')
            ->get();
        
        // Get subjects for the partner
        $subjects = Subject::where('partner_id', $partnerId)
            ->orderBy('name')
            ->get();
        
        // Get topics for the partner
        $topics = Topic::where('partner_id', $partnerId)
            ->orderBy('name')
            ->get();
        
        // Get question types
        $questionTypes = QuestionType::orderBy('q_type_name')->get();
        
        // Get exams for the partner
        $exams = \App\Models\Exam::where('partner_id', $partnerId)
            ->orderBy('title')
            ->get();
        
        return view('partner.questions.download', compact('courses', 'subjects', 'topics', 'questionTypes', 'exams'));
    }

    /**
     * Get exam details for AJAX request
     */
    public function getExamDetails($id)
    {
        ...
        return response()->json([
            ...
            'paper_parameter' => $exam->title . ' - ' . $exam->exam_type
        ]);
    }


    /**
     * Process question download with paper parameter
     */
    public function downloadQuestions(Request $request)
    {
        ...
        $request->validate([
            ...
            'topic_id' => 'nullable|exists:topics,id',
            'question_type_id' => 'nullable|exists:question_types,q_type_id',
            'paper_parameter' => 'required|string|max:255',
            'format' => 'required|in:csv,excel,pdf'
        ]);
        
        try {
            // Build query for questions
            $query = Question::with(['course', 'subject', 'topic', 'questionType', 'partner'])
                ->where('partner_id', $partnerId)
            ...
            if ($questions->isEmpty()) {
                return redirect()->back()->with('error', 'No questions found with the selected criteria.');
            }
            
            $paperParameter = $request->paper_parameter;
            $format = $request->format;
            
            // Generate filename with paper parameter
            $course = Course::find($request->course_id);
            $filename = $course->name . '_' . $paperParameter . '_questions.' . $format;
            $filename = preg_replace('/[^a-zA-Z0-9_-]/', '_', $filename); // Sanitize filename
            
            if ($format === 'csv') {
                return $this->downloadCsv($questions, $filename, $paperParameter);
            } elseif ($format === 'excel') {
                return $this->downloadExcel($questions, $filename, $paperParameter);
            } elseif ($format === 'pdf') {
                return $this->downloadPdf($questions, $filename, $paperParameter);
            }
            
        } catch (\Exception $e) {
            \Log::error('Error in downloadQuestions: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Failed to download questions: ' . $e->getMessage());
        }
    }
     ...
     */
    private function downloadCsv($questions, $filename, $paperParameter)
    {
        $headers = [
        ...
        $callback = function() use ($questions, $paperParameter) {
            $file = fopen('php://output', 'w');
            
            // Add paper parameter as header
            fputcsv($file, ['Paper Parameter: ' . $paperParameter]);
            ...
            foreach ($questions as $question) {
                $options = '';
                if ($question->questionType && $question->questionType->q_type_name === 'MCQ') {
                    $options = json_encode([
                        'A' => $question->option_a,
                        'B' => $question->option_b,
                        'C' => $question->option_c,
                ...
                fputcsv($file, [
                    ...
                    $question->topic->name ?? '',
                    $question->questionType->q_type_name ?? '',
     ...
     */
    private function downloadExcel($questions, $filename, $paperParameter)
    {
        // For now, return CSV format as Excel is not implemented
        // You can implement Excel export using Laravel Excel package
        return $this->downloadCsv($questions, str_replace('.xlsx', '.csv', $filename), $paperParameter);
    }
    
    /**
     * Download questions as PDF
     */
    private function downloadPdf($questions, $filename, $paperParameter)
    {
        // For now, return CSV format as PDF is not implemented
        // You can implement PDF export using DomPDF or similar
        return $this->downloadCsv($questions, str_replace('.pdf', '.csv', $filename), $paperParameter);
    }
}
