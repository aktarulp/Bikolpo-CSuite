...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div class="bg-gradient-to-br from-primaryGreen/10 to-emerald-50 dark:from-primaryGreen/20 dark:to-emerald-900/30 rounded-xl p-4 border border-primaryGreen/20 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 text-xs">Total Exams</p>
                </div>
            </div>
        </div>

        <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800/50 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 text-xs">Available Now</p>
                </div>
            </div>
        </div>

        <div class="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/30 dark:to-amber-800/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800/50 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 text-xs">Completed</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="lg:hidden">
                <div class="divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach($assignedExams as $exam)
                        <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors duration-200">
                            <div class="flex justify-between items-start">
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-bold text-gray-900 dark:text-white text-sm truncate">{{ $exam->title }}</h3>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">ID: {{ $exam->id }}</p>
                            ...
                            <div class="mt-3 flex flex-wrap gap-2">
                                ...
                                @elseif($isUpcoming)
                                    <span class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-lg text-amber-700 bg-amber-100 dark:bg-amber-900 dark:text-amber-200">
                                        ...
                                        <span id="countdown-{{ $exam->id }}" data-start-time="{{ $exam->start_time->timestamp }}">
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-750">
                        <tr>
                            <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Exam ID</th>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Schedule</th>
                            <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Questions</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @foreach($assignedExams as $exam)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors duration-200">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-white max-w-xs truncate">
                                    {{ $exam->title }}
                                </td>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm">
                                    ...
                                    @if($canTakeExam)
                                        <a href="{{ route('student.exams.start', $exam) }}" 
                                           class="inline-flex items-center px-2.5 py-1.5 bg-gradient-to-r from-primaryGreen to-emerald-600 hover:from-primaryGreen/90 hover:to-emerald-700 text-white text-xs font-medium rounded-lg shadow transition-all duration-300">
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                    ...
                                    @elseif($isUpcoming)
                                        <span class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium rounded-lg text-amber-700 bg-amber-100 dark:bg-amber-900 dark:text-amber-200">
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium rounded-lg text-gray-500 bg-gray-100 dark:bg-gray-700">
                                            @if($exam->status !== 'published')
                                                Not Published
                                            @else
                                                Not Available
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm">
                                    ...
                                    @if($result)
                                        @if($result->status === 'completed')
                                            <a href="{{ route('student.exams.result', $exam) }}" 
                                               class="inline-flex items-center px-2.5 py-1.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-xs font-medium rounded-lg shadow transition-all duration-300">
                                                Result
                                            </a>
                                        @elseif($result->status === 'in_progress')
                                            <span class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium rounded-lg text-amber-700 bg-amber-100 dark:bg-amber-900 dark:text-amber-200">
