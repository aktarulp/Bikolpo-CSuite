...
<body class="font-sans antialiased">
    <div class="min-h-screen bg-gray-100 flex">
        <!-- Left Sidebar (exact from layouts/dashboard) -->
        ...
        <div class="flex-1 flex flex-col">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                <!-- Professional Welcome Banner with Navigation Tabs (from partner/dashboard) -->
                    ...
                    <div class="px-8 py-6">
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div>
                                        <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">
                                            Welcome back, <span class="text-blue-600 dark:text-blue-400">{{ optional($partner ?? null)->slug ?? optional($partner ?? null)->name ?? (Auth::user()->name ?? 'Partner') }}</span>
                                        </h2>
                                ...
                                <div class="relative group">
                                    <button class="flex items-center space-x-2 p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-all duration-200 focus:outline-none">
                                        <div class="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center overflow-hidden">
                                            @if(!empty(optional($partner ?? null)->logo))
                                                <img src="{{ asset('storage/' . optional($partner ?? null)->logo) }}" alt="Partner Logo" class="w-full h-full object-cover">
                                            @else
                                                <span class="text-sm font-bold text-white">{{ substr(optional($partner ?? null)->slug ?? optional($partner ?? null)->name ?? (Auth::user()->name ?? 'P'), 0, 1) }}</span>
                                            @endif
                                        </div>
                        ...
                        <div class="border-t border-gray-200 dark:border-gray-700">
                            <nav class="flex items-center space-x-1">
                               <a href="{{ route('partner.dashboard') }}" 
                                  class="px-4 py-2 text-sm font-medium {{ request()->routeIs('partner.dashboard') ? 'text-primaryGreen bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-all duration-200' }}">
                                   Overview
                               </a>
                               <a href="{{ route('partner.questions.all') }}" 
                                  class="px-4 py-2 text-sm font-medium {{ request()->routeIs('partner.questions.*') ? 'text-primaryGreen bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-all duration-200' }}">
                                   Questions
                               </a>
                               <a href="{{ route('partner.exams.index') }}" 
                                  class="px-4 py-2 text-sm font-medium {{ request()->routeIs('partner.exams.*') ? 'text-primaryGreen bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-all duration-200' }}">
                               ...
                               <a href="{{ route('partner.students.index') }}" 
                                  class="px-4 py-2 text-sm font-medium {{ request()->routeIs('partner.students.*') ? 'text-primaryGreen bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-all duration-200' }}">
                                   Students
                               </a>
                           </nav>
                        </div>
    ...
    <!-- JavaScript for Sidebar Toggle and Mobile Sidebar + Dark Mode Toggle -->
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function updateSidebar() {
                if (sidebarOpen) {
                    ...
                    if (logoText) { logoText.style.opacity = '1'; logoText.style.display = 'block'; }
                    if (userInfo) { userInfo.style.opacity = '1'; userInfo.style.display = 'block'; }
                    if (logoutButton) { logoutButton.style.opacity = '1'; logoutButton.style.display = 'block'; }
                    ...
                    if (logoText) { logoText.style.opacity = '0'; logoText.style.display = 'none'; }
                    if (userInfo) { userInfo.style.opacity = '0'; userInfo.style.display = 'none'; }
                    if (logoutButton) { logoutButton.style.opacity = '0'; logoutButton.style.display = 'none'; }
            ...
            if (profileMenuToggle && profileSubmenu && profileArrow) {
                ...
                // Auto-expand submenu if on profile page
