...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-4xl font-bold text-gray-900 dark:text-white mb-2">Edit Exam</h1>
            <p class="text-lg text-gray-600 dark:text-gray-300">Update your exam configuration and settings</p>
        </div>
        ...
        <!-- Form -->
        <form id="examForm" action="{{ route('partner.exams.update', $exam) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" 
                                      ...
                                      class="w-full px-4 py-4 border-2 border-gray-200 dark:border-gray-600 rounded-2xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 dark:bg-slate-700 dark:text-white transition-all duration-300 resize-none">{{ old('description', $exam->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="hidden" id="questionHeader" name="question_header" value="{{ old('question_header', $exam->question_header) }}">
                            </div>
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="datetime-local" id="startDateTime" name="startDateTime" 
                               value="{{ old('startDateTime', $exam->start_time ? $exam->start_time->format('Y-m-d\TH:i') : now()->addDay()->format('Y-m-d\TH:i')) }}" required 
                    ...
                    <div>
                        ...
                        <input type="datetime-local" id="endDateTime" name="endDateTime" 
                               value="{{ old('endDateTime', $exam->end_time ? $exam->end_time->format('Y-m-d\TH:i') : now()->addDay()->addHours(2)->format('Y-m-d\TH:i')) }}" required 
                               class="w-full px-4 py-4 border-2 border-gray-200 dark:border-gray-600 rounded-2xl focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 dark:bg-slate-700 dark:text-white transition-all duration-300 text-lg" 
                               onchange="validateDateTime()">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-700 rounded-2xl">
                            ...
                            <label class="toggle-switch">
                                <input type="checkbox" id="allowRetake" name="allow_retake" value="1" {{ old('allow_retake', $exam->allow_retake) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-700 rounded-2xl">
                            ...
                            <label class="toggle-switch">
                                <input type="checkbox" id="showResults" name="show_results_immediately" value="1" {{ old('show_results_immediately', $exam->show_results_immediately) ? 'checked' : '' }}>
                                <span class="slider"></span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="p-4 bg-gray-50 dark:bg-slate-700 rounded-2xl">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <label class="toggle-switch">
                                    <input type="checkbox" id="negativeMarking" name="has_negative_marking" value="1" {{ old('has_negative_marking', $exam->has_negative_marking) ? 'checked' : '' }} onchange="toggleNegativeMarkingInput(this)">
                                    <span class="slider"></span>
                                </label>
                            </div>
                            <div id="negativeMarkingInputContainer" class="{{ old('has_negative_marking', $exam->has_negative_marking) ? '' : 'hidden' }}">
                                ...
                                <input type="number" id="negativeMarkingInput" name="negative_marks_per_question" 
                                       value="{{ old('negative_marks_per_question', $exam->negative_marks_per_question) }}" min="0" max="5" step="0.25" 
            ...
            <div class="form-section bg-white dark:bg-slate-800 rounded-3xl shadow-xl p-8 card-hover">
                <div class="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                    <div class="text-center sm:text-left">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Ready to update your exam?</h3>
...
<script>
    ...
    function initializeQuestionHeaderEditor() {
        ...
        toolbar.innerHTML = `
            ...
            <button type="button" class="p-2 text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-slate-500 rounded transition-colors" onclick="execCommand('justifyCenter')" title="Align Center">
